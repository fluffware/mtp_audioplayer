...
impl Scheduler {
    ...
    fn get_token_with_notify(self: &Arc<Scheduler>, priority: i32) -> (Token, Arc<Notify>) {
...
async fn test_equal_prority() {
    ...
    tokio::spawn(async move {
        ...
        match tokio::time::timeout(Duration::from_millis(500), token1.wait_release()).await {
    ...
    tokio::spawn(async move {
        ...
        match tokio::time::timeout(Duration::from_millis(500), token2.wait_release()).await {
    ...
    tokio::spawn(async move {
        ...
        match tokio::time::timeout(Duration::from_millis(500), token.wait_release()).await {
...
async fn test_higher_prority() {
    ...
    tokio::spawn(async move {
        let token1 = sched1.get_token(3).await;
        match tokio::time::timeout(Duration::from_millis(3000), token1.wait_release()).await {
            Ok(_) => println!("Forced release token1"),
            Err(_) => println!("Release token1"),
        }
    });
    ...
    tokio::spawn(async move {
        ...
        tokio::time::sleep(Duration::from_millis(500)).await;
        match tokio::time::timeout(Duration::from_millis(1000), token2.wait_release()).await {
            Ok(_) => println!("Forced release token2"),
            Err(_) => println!("Release token2"),
        }
    });
